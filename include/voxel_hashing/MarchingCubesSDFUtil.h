...
struct MarchingCubesData {
	...
	void extractIsoSurfaceAtPosition(const float3& worldPos, const HashData& hashData)
	{
		...
		// if (cubeindex != 0) {
		// 	printf("cubeindex: %d, dist000: %f, dist100: %f, dist010: %f, dist001: %f, dist110: %f, dist011: %f, dist101: %f, dist111: %f\n", cubeindex, dist000, dist100, dist010, dist001, dist110, dist011, dist101, dist111);
		// }
	...
	bool trilinearInterpolationSimpleFastFast(const HashData& hash, const float3& pos, float& dist) const {
		...
		dist = 0.0f;
		Voxel* v = hash.getVoxel(posDual+make_float3(0.0f, 0.0f, 0.0f)); if(v == NULL) return false; if((int)v->weight_sum == 0) return false; dist+= (1.0f-weight.x)*(1.0f-weight.y)*(1.0f-weight.z)*v->sdf_sum/(int)v->weight_sum;
		v = hash.getVoxel(posDual+make_float3(oSet, 0.0f, 0.0f)); if(v == NULL) return false; if((int)v->weight_sum == 0) return false; dist+=	   weight.x *(1.0f-weight.y)*(1.0f-weight.z)*v->sdf_sum/(int)v->weight_sum;
		v = hash.getVoxel(posDual+make_float3(0.0f, oSet, 0.0f)); if(v == NULL) return false; if((int)v->weight_sum == 0) return false; dist+= (1.0f-weight.x)*	   weight.y *(1.0f-weight.z)*v->sdf_sum/(int)v->weight_sum;
		v = hash.getVoxel(posDual+make_float3(0.0f, 0.0f, oSet)); if(v == NULL) return false; if((int)v->weight_sum == 0) return false; dist+= (1.0f-weight.x)*(1.0f-weight.y)*	   weight.z *v->sdf_sum/(int)v->weight_sum;
		v = hash.getVoxel(posDual+make_float3(oSet, oSet, 0.0f)); if(v == NULL) return false; if((int)v->weight_sum == 0) return false; dist+=	   weight.x *	   weight.y *(1.0f-weight.z)*v->sdf_sum/(int)v->weight_sum;
		v = hash.getVoxel(posDual+make_float3(0.0f, oSet, oSet)); if(v == NULL) return false; if((int)v->weight_sum == 0) return false; dist+= (1.0f-weight.x)*	   weight.y *	   weight.z *v->sdf_sum/(int)v->weight_sum;
		v = hash.getVoxel(posDual+make_float3(oSet, 0.0f, oSet)); if(v == NULL) return false; if((int)v->weight_sum == 0) return false; dist+=	   weight.x *(1.0f-weight.y)*	   weight.z *v->sdf_sum/(int)v->weight_sum;
		v = hash.getVoxel(posDual+make_float3(oSet, oSet, oSet)); if(v == NULL) return false; if((int)v->weight_sum == 0) return false; dist+=	   weight.x *	   weight.y *	   weight.z *v->sdf_sum/(int)v->weight_sum;
